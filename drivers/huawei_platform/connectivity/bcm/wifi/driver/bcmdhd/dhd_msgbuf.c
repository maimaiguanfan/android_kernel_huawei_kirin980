...
dhd_prot_rxbuf_post(dhd_pub_t *dhd, uint16 count, bool use_rsv_pktid)
{
	...
	if (alloced > 0) {
		...
		if (dhd->bus->is_linkdown) dhd_os_send_hang_message(dhd);
#endif
	}
...
#ifdef BCM_WAKELOCK_PATCH
#define TX_WAKE_TIMEOUT_IN_JIFFIES msecs_to_jiffies(20000) /* 20s */
...
#ifdef HW_PCIE_STABILITY
		//before read avoid pcie noc
		dhd_bus_cmn_readshared(dhd->bus, &wr,
			RING_WR_UPD, ring->idx);
		if (wr == (uint16)-1) {
			dhd->bus->is_linkdown = 1;
			dhd->hang_reason = HANG_REASON_PCIE_LINK_DOWN;
#ifdef HW_WIFI_DMD_LOG
			hw_wifi_dsm_client_notify(DSM_WIFI_CMD53_ERROR, "%s: read FFFF error\n", __FUNCTION__);
#endif
			DHD_ERROR(("%s: Device removed. deepsleep=%d\n", __func__,
				   dhd->deepsleep)); 
			DHD_ERROR(("%s: ring(%d) init(%d) rd addr(%x) wr addr(%x)\n", __func__,
				   ring->idx, ring->inited,
				   dhd->bus->ring_sh[ring->idx].ring_state_r,
				   dhd->bus->ring_sh[ring->idx].ring_state_w));
			DHD_ERROR(("%s: RD %d CURR_RD %d WR %d BASE(VA) %p BASE(PA) %x:%x SIZE %d\n", __func__,
				   ring->rd, ring->curr_rd, ring->wr, ring->dma_buf.va,
				   ltoh32(ring->base_addr.high_addr),
				   ltoh32(ring->base_addr.low_addr),
				   ring->max_items * ring->item_len));
			DHD_ERROR(("%s: active_tx_count %d pktidmap_avail %d\n", __func__,
				   prot->active_tx_count,
				   DHD_PKTID_AVAIL(prot->pktid_map_handle)));
			return;
		}
#endif
		dhd_bus_cmn_writeshared(dhd->bus, &(ring->wr),
