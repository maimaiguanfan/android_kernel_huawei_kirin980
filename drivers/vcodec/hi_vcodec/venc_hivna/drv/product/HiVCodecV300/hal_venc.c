...
HI_VOID VeduHal_CfgReg_SMMUSet(HI_VOID)
{
    ...
    if (gVencQosMode == 1) {
        pAllReg->SMMU_SCR.bits.rqos = 2;
        pAllReg->SMMU_SCR.bits.wqos = 2;
    }
    ...
    pAllReg->SMMU_FAMA_CTRL1_NS.bits.fama_ptw_msb_ns = (phy_pgd_base >> 32) & 0x7F;
