...
int fiq_init(void)
{
	...
	(void)memset_s ((void *)&g_fiq_ctrl, sizeof(struct fiq_ctrl), 0, sizeof(struct fiq_ctrl));
	g_fiq_ctrl.smem_base_addr = (void *)SHM_FIQ_BASE_ADDR;
        /*coverity[secure_coding]*/
	(void)memset_s(g_fiq_ctrl.smem_base_addr,  SHM_SIZE_CCORE_FIQ, 0, SHM_SIZE_CCORE_FIQ);
